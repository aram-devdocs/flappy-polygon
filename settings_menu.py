...
class Toggle:
    ...
    def draw(self, surface):
        ...
        label_surface = self.font.render(f"{self.label}: {'On' if self.state else 'Off'}", True, (0, 0, 0))
        surface.blit(label_surface, (self.rect.x + 50, self.rect.y))

class SettingsMenu:
    def __init__(self, width, height, bird_jump_strength, pipe_speed, training_mode=False):
        self.width = width
        self.height = height
        ...
        self.training_steps_slider = Slider(100, 350, 200, 1000, 50000, 10000, "Training Steps")
        self.learning_rate_slider = Slider(100, 450, 200, 0.0001, 0.01, 0.001, "Learning Rate")

        # Training mode toggle
        self.training_mode_toggle = Toggle(100, 550, "Training Mode", initial_state=training_mode)

        self.sliders = [self.jump_strength_slider, self.pipe_speed_slider, self.training_steps_slider, self.learning_rate_slider]
        self.active = False
